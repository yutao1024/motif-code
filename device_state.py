...
class DeviceState(object):
    ...
    def __get_hashed_state_str(self):
        state, _, _ = self.get_described_actions(remove_time_and_ip=True)
        # state = self.get_number_free_screen()
    ...
    def _extract_all_children(self, id):
        ...
        # return childrenlist
    def _merge_textv2(self, children_ids, remove_time_and_ip=False):
    ...
    def get_described_actions(self, prefix='', remove_time_and_ip=False):
        ...
        for view_id in enabled_view_ids:
            ...
            if editable:
                ...
                view_text, content_description = self._merge_textv2(clickable_children_ids, remove_time_and_ip)
                ...
                for clickable_child in clickable_children_ids:
                    if clickable_child in enabled_view_ids and clickable_child != view_id:
                        ...
                        # enabled_view_ids.remove(clickable_child)
                        # print('origin:', view_id, 'because of:', clickable_ancestor_id, 'removed:', clickable_child)
    ...
    #     # note that we also do not include clicked or checked
    #     enabled_view_ids = []
    #     for view_dict in self.views:
    #         # exclude navigation bar if exists
    #         if self.__safe_dict_get(view_dict, 'visible') and \
    ...
    #         if editable:
    #             # view_status += 'editable '
    #             view_desc = input_frame.replace('@', str(len(view_descs)))
    #             if content_description:
    #                 view_desc = view_desc.replace('&', content_description)
    #             else:
    #                 view_desc = view_desc.replace(" class='&'", "")
    ...
    #         elif (clickable or checkable or long_clickable):
    #             view_desc = btn_frame.replace('@', str(len(view_descs)))#.replace('$', str(checked or selected))
    #             if content_description:
    #                 view_desc = view_desc.replace('&', content_description)
    #             else:
    #                 view_desc = view_desc.replace(" class='&'", "")
    ...
    #         else:
    #             view_desc = text_frame.replace('@', str(len(view_descs)))
    #             if content_description:
    #                 view_desc = view_desc.replace('&', content_description)
    #             else:
    #                 view_desc = view_desc.replace(" class='&'", "")
